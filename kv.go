...
type KIRV struct {
	M map[int]*RV // Could we use int or string keys and that would make things faster?
}

// Get 
func (kv *KIRV) Get(k int) (io.Reader, error) {
	// Make sure the k is in the list of keys
	if v, ok := kv.M[k]; ok {
		// If the data is not ready then wait until it is
		if !v.Ready {
...
func (kv *KIRV) SetPRF(k int, prf RVFunc, data io.Reader) {
	v := kv.set(k)
	go func() {
		d := prf(data)
		v.Queue <- d
